...
(llama-defun save-session (ctx path &optional tokens)
...
TOKENS is an optional vector of token integers to store alongside the state."
  (restart-case
    ...
    (use-different-path (new-path)
      :report "Retry saving session to a different path"
      :interactive (lambda ()
                     (format *query-io* "Session path: ")
                     (list (read-line *query-io*)))
      (save-session ctx new-path tokens))
    (skip-save ()
      :report "Continue without saving session state"
      nil)))

(llama-defun load-session (ctx path)
  ...
  (restart-case
      (let* ((ctx-ptr (llama-context-pointer ctx))
        ...
        (unwind-protect
          ...
          (cffi:foreign-free tok-buf)))
    (use-different-path (new-path)
      :report "Retry loading session from a different path"
      :interactive (lambda ()
                     (format *query-io* "Session path: ")
                     (list (read-line *query-io*)))
      (load-session ctx new-path))
    (start-fresh ()
      :report "Continue without loading saved session state (start fresh)"
      (make-array 0 :element-type 'fixnum))))

(llama-defun save-session-seq (ctx path seq-id &optional tokens)
  ...
  (restart-case
    ...
    (use-different-path (new-path)
      ...
      :interactive (lambda ()
                     (format *query-io* "Session path: ")
                     (list (read-line *query-io*)))
      (save-session-seq ctx new-path seq-id tokens))
    (skip-save ()
      :report "Continue without saving session state"
      nil)))

(llama-defun load-session-seq (ctx path seq-id)
  ...
  (restart-case
    ...
    (use-different-path (new-path)
      ...
      :interactive (lambda ()
                     (format *query-io* "Session path: ")
                     (list (read-line *query-io*)))
      (load-session-seq ctx new-path seq-id))
    (start-fresh ()
      :report "Continue without loading saved session state (start fresh)"
      (make-array 0 :element-type 'fixnum))))

(llama-defun save-state (ctx)
...
(llama-defun load-state (ctx state-bytes)
...
(llama-defun save-state-seq (ctx seq-id &key flags)
...
(llama-defun load-state-seq (ctx seq-id state-bytes &key flags)

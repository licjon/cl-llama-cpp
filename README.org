...
- *SBCL* (recommended) — llama.cpp triggers floating-point exceptions that SBCL masks
  automatically. Other implementations will need to handle FP masking and may require
  additional work for multithreading. See =with-llama-compatible-fp-environment=.
